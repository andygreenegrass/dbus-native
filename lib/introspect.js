      ...
      parser.parseString(xml, function(err, result) {
        ...
        for (let i = 0; i < ifaces.length; ++i) {
          ...
          (function() {
            ...
            currentIface.addListener = currentIface.on = (function(ifName) {
              return function(signame, callback) {
                ...
                const signalFullName = bus.mangle(obj.name, ifName, signame);

                if (!bus.signals.listeners(signalFullName).length) {
                  // This is the first time, so call addMatch
                  const match = getMatchRule(obj.name, ifName, signame);
          ...
          for (let p = 0; iface.property && p < iface.property.length; ++p) {
            ...
            //TODO: move up
            const addReadProp = function(iface, ifName, propName, property) {
